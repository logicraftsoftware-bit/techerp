                    ...
                    <div x-data="{ open: {{ $groupOpen ? 'true' : 'false' }} }" class="pt-2">
                        ...
                        <div x-show="open" class="space-y-1">
                            @foreach($items as [$slug, $label, $description])
                                @php $resource = ['customers','machines','technicians','skills']; $active = in_array($slug,$resource) ? request()->routeIs($slug.'.*') : (request()->routeIs('modules.show') && request()->route('module') === $slug); @endphp
