...
<div class="mx-auto max-w-xl">
    ...
    @php $currentStaff = $leave->technician_id ? 'technician:'.$leave->technician_id : 'user:'.$leave->user_id; @endphp
    <form method="POST" action="{{ route('leave.update-details', $leave) }}" class="card grid gap-4 p-6">
        @csrf
        @method('PUT')
        <div><label class="form-label">Staff *</label><select class="form-input" name="staff" required><option value="">Staff</option>@foreach($staff as $x)<option value="{{ $x->key }}" @selected(old('staff', $currentStaff) === $x->key)>{{ $x->code }} — {{ $x->name }}</option>@endforeach</select></div>
        ...
        <div class="flex justify-end gap-3"><a href="{{ route('leave.index') }}" class="btn-secondary">Cancel</a><button class="btn-primary">Save Changes</button></div>
    </form>
</div>
@endsection
