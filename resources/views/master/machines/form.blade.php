...
<div><label class="form-label">Machine Category *</label><select name="machine_category_id" class="form-input" required><option value="">Select category</option>@foreach($categories as $category)<option value="{{$category->id}}" @selected(old('machine_category_id',$machine->machine_category_id)==$category->id)>{{$category->category_name}}</option>@endforeach</select><a class="mt-1 inline-block text-xs text-blue-600" href="{{route('machine-categories.create')}}">+ Add category</a></div>
