...
<div><label class="form-label">Machine Photos</label><input class="form-input" type="file" name="machine_photos[]" accept="image/*" multiple><p class="mt-1 text-xs text-slate-400">Upload up to 10 photos.</p></div>
<div><label class="form-label">Warranty Card</label><input class="form-input" type="file" name="warranty_card" accept="image/*,.pdf"></div>
<div><label class="form-label">Service Coupon</label><input class="form-input" type="file" name="service_coupon" accept="image/*,.pdf"></div>
</div></section>
@if($machine->exists && $machine->documents->isNotEmpty())<section class="card mt-6 p-6"><h3 class="font-bold">Existing uploads</h3><div class="mt-4 flex flex-wrap gap-2">@foreach($machine->documents as $document)<a class="btn-secondary" target="_blank" href="{{Storage::url($document->file_path)}}">{{$document->original_name}}</a>@endforeach</div></section>@endif
