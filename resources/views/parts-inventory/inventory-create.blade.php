...
<form method="POST" action="{{ route('inventory.store') }}" class="mx-auto max-w-4xl">
    ...
    <section class="card p-6">
        <div class="grid gap-5 md:grid-cols-2">
            ...
            <div><label class="form-label">Supplier</label><select name="supplier_id" class="form-input"><option value="">Supplier</option>@foreach($suppliers as $x)<option value="{{ $x->id }}" @selected(old('supplier_id') == $x->id)>{{ $x->company_name }}</option>@endforeach</select></div>
